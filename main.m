    ...
    for chn = 1:length(ch)
        ...
        %        epo = BCI_MI_train_se1(:,:,:,sub); % Training data : 400 Time series x 66 channels x 100 trials 
         ...
         %% TEST data load
            ...
            tm(a,1) = 2; tm(b,1) = 1;       
            Y_pred1(:, sub) = tm;
            [a] = find(BCI_MI_train_se1_Y(1:20,sub) == Y_pred1(:,sub));
            tempacc=length(a)/20;
            if(tempacc>acc)
                besttime = t;
                acc = tempacc;
...
clearvars -except besttimelist BCI_MI_train_se1_Y BCI_MI_train_se1 BCI_MI_test_se1 BCI_MI_test_se1_Y fs bandr ch ivals BTB
